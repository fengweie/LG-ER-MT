...
parser.add_argument('--exp', type=str,  default='UAMT_all_noise_discrimitive_alldata', help='model_name')
...
parser.add_argument('--discrimitive', type=float,  default=0.00001, help='discrimitive')
...
#    shutil.copytree('.', snapshot_path + '/code', shutil.ignore_patterns(['.git','__pycache__']))
    ...
    for epoch_num in tqdm(range(max_epoch), ncols=70):
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            # emaoutputs_soft = F.softmax(ema_output, dim=1)
            emaoutputs_soft = F.softmax(ema_output, dim=1)
            unlabal_hard = torch.max(emaoutputs_soft[labeled_bs:],1)[1]
            merge_hard = torch.cat((unlabal_hard,label_batch[:labeled_bs]),0)
            discrimitive_value = losses.discrimitive_loss(x9_feature, merge_hard)
            ...
            loss = 0.5*(loss_seg+loss_seg_dice) + consistency_loss + loss_discrimitive
